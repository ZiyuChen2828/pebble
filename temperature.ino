...
void loop()  { 
  ...
  while (1) {
    ...
    Temperature_F = Temperature_H * 1.8 + 32;
//    Serial.println(Temperature_F,DEC);
    /* Calculate temperature */
    if (farenheitOn){
      Cal_temp (Decimal, Temperature_F, Temperature_L, IsPositive);
    ...
    if (!standby){
        if (farenheitOn){
          Dis_7SEG (Decimal, Temperature_F, Temperature_L, IsPositive);
        }
        else{
